...
for K in [""]:
    for index in ["00000001", "00000020", "00000030", "00000040", "00000050"  ]:
        ...
        mc_4_avg_bottom_y = []
        count = 0
        x_width = 100 # np.max(x_pos_bottom)
        y_width = 100 #np.max(y_pos_bottom)
        x_min = 0
        y_min = 0
        ...
        for length in mag_data[2]:
            if(length == 0):
                ...
                mc_1_avg_bottom.append(mag_data[5][count]*mag_data[6][count]+mag_data[10][count]*mag_data[11][count])
                
                mc_1_avg_bottom_x.append(mag_data[3][count]*mag_data[6][count]+mag_data[8][count]*mag_data[11][count])
            
                mc_1_avg_bottom_y.append(mag_data[4][count]*mag_data[6][count]+mag_data[9][count]*mag_data[11][count])
        ...
        grid_x, grid_y = np.mgrid[x_min:x_width:150j, y_min:y_width:150j]
        ...
        mag_min = -2 #np.min(grid_1z_avg)
        mag_max = 2 #abs(np.max(grid_1z_avg))
        if(mag_min < 0.0):
        ...
        plt.imshow(grid_mx_avg, extent=(0,x_width, 0,x_width), origin='lower',cmap=cmap1,norm=norm1, interpolation='gaussian' )
        ...
        plt.imshow(grid_my_avg, extent=(0,x_width, 0,x_width), origin='lower',cmap=cmap1,norm=norm1, interpolation='gaussian' )
        
        plt.subplot(1,4, 3)
        plt.xticks([])
        plt.yticks([])
        #grid_J1 = griddata((x_pos_bottom,y_pos_bottom), mc_1_avg_bottom, (grid_x, grid_y), fill_value=0.0, method="cubic")
        plt.imshow(grid_m_avg, extent=(0,x_width, 0,x_width), origin='lower',cmap=cmap1,norm=norm1, interpolation='gaussian' )
        ...
        plt.yticks([])
        #grid_J1 = griddata((x_pos_bottom,y_pos_bottom), mc_1_avg_bottom, (grid_x, grid_y), fill_value=0.0, method="cubic")
        plt.imshow(grid_alloy, extent=(0,x_width, 0,x_width), origin='lower',cmap=cmap1,norm=norm1, interpolation='gaussian' )
        #plt.colorbar( cmap = cmap1, shrink = 0.5, aspect= 10, norm=norm1 )

        #plt.savefig(param + "-" + K + "-" + index +"-z-layers-avg-ori.png", bbox_inches = 'tight')
        plt.savefig("5K MC -> LLG, 500mT" + index +"-pinning-5nm-smth-1-4L-1nm.png", bbox_inches = 'tight')
        plt.clf()
