...
for K in [""]:
    for i in range(10,150,10):
        ...
        mc_4_avg_bottom_y = []
        count = 0
        x_width = 40 # np.max(x_pos_bottom)
        y_width = 40 #np.max(y_pos_bottom)
        x_min = 0
        y_min = 0
        ...
        for length in mag_data[2]:
            if(length == 0):
                ...
                mc_1_avg_bottom.append(mag_data[25][count]*mag_data[26][count])
                
                mc_1_avg_bottom_x.append(mag_data[23][count]*mag_data[26][count])
            
                mc_1_avg_bottom_y.append(mag_data[24][count]*mag_data[26][count])
        ...
        plt.yticks([])
        #grid_J1 = griddata((x_pos_bottom,y_pos_bottom), mc_1_avg_bottom, (grid_x, grid_y), fill_value=0.0, method="cubic")
        plt.imshow(grid_alloy.T, extent=(0,x_width, 0,x_width), origin='lower',cmap=cmap1,norm=norm1, interpolation='gaussian' )
        #plt.colorbar( cmap = cmap1, shrink = 0.5, aspect= 10, norm=norm1 )

        #plt.savefig(param + "-" + K + "-" + index +"-z-layers-avg-ori.png", bbox_inches = 'tight')
        plt.savefig("FGT-FC-MC-500mT" + index +"-pinning-1.5nm-4L.png", bbox_inches = 'tight')
        plt.clf()
