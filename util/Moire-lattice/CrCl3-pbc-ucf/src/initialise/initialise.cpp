...
bool DMI = false;
bool BQ = false;
bool bake_only = false;
bool write_config_output = false; // --config-atoms: dump config_energy_*.txt (slow)
int moire_force_candidate = -1; // --candidate N: try only that ranked AA quartet (0-based)
...
int moire_period_Px = 0;
int moire_period_Py = 0;
int moire_period_i0 = 0;
int moire_period_j0 = 0;
bool moire_period_valid = false;
double moire_origin_x = 0, moire_origin_y = 0;
double moire_tx_x = 0, moire_tx_y = 0, moire_ty_x = 0, moire_ty_y = 0;
double moire_ucf_c = 1.0, moire_ucf_s = 0.0;
std::vector < spin > moire_primary_atoms;
...
void initialise_variables(){
   ...
   char directory [256];
      ...
      // Biquadratic file only when --bq; remove stale files otherwise so generate.sh won't append them
      const std::string bq_path = std::string(directory) + "/interactions_biquadratic.ucf";
      const std::string bq_hdr  = std::string(directory) + "/header_interactions_biquadratic.ucf";
      if(BQ) {
         outfile_bq.open(bq_path);
      } else {
         std::remove(bq_path.c_str());
         std::remove(bq_hdr.c_str());
      }
   ...
   // microcell_scale_x = 2;
   // microcell_scale_y = 2;
   microcell_Nx = number_of_unit_cells_x / microcell_scale_x;
   microcell_Ny = number_of_unit_cells_y / microcell_scale_y;

   std::cout << "Creating base lattice " << number_of_unit_cells_x << " by " << number_of_unit_cells_y << " rhombic unit cells"
             << "; microcells " << microcell_Nx << " x " << microcell_Ny << " (scale " << microcell_scale_x << " x " << microcell_scale_y << ")" << std::endl;
   ...
   default_shift[1] = 67;
   default_shift[2] = 0;
   unit_cell_shifts.resize(microcell_Nx + 1);
   for(int i = 0; i <= microcell_Nx; i++) {
      unit_cell_shifts[i].resize(microcell_Ny + 1);
      for(int j = 0; j <= microcell_Ny; j++) {
         unit_cell_shifts[i][j].resize(3, 0);
      }
   }

   global_config_energy.resize(microcell_Nx + 1);
   for(int i = 0; i <= microcell_Nx; i++) {
      global_config_energy[i].resize(microcell_Ny + 1);
      for(int j = 0; j <= microcell_Ny; j++) {
         global_config_energy[i][j].resize(4*16,0.0);
      }
   }
