...
namespace anisotropy{
   ...
   void identify_surface_atoms(std::vector<cs::catom_t> & catom_array, std::vector<std::vector <neighbours::neighbour_t> > & cneighbourlist){
      ...
      std::vector<bool> nn_interaction(cs::unit_cell.bilinear.interaction_count,false);
      ...
      for(uint64_t itr = 0; itr < cs::unit_cell.bilinear.interaction_count; itr++){

         // get distance to neighbouring unit cell in unit cells
         double nndx = double(cs::unit_cell.bilinear.interaction_ptr[itr].dx);
         double nndy = double(cs::unit_cell.bilinear.interaction_ptr[itr].dy);
         double nndz = double(cs::unit_cell.bilinear.interaction_ptr[itr].dz);

         // load positions of i and j atoms to temporary coordinates and convert to angstroms
         double ix = (cs::unit_cell.atom[cs::unit_cell.bilinear.interaction_ptr[itr].i].x) * ucdx;
         double iy = (cs::unit_cell.atom[cs::unit_cell.bilinear.interaction_ptr[itr].i].y) * ucdy;
         double iz = (cs::unit_cell.atom[cs::unit_cell.bilinear.interaction_ptr[itr].i].z) * ucdz;
         double jx = (cs::unit_cell.atom[cs::unit_cell.bilinear.interaction_ptr[itr].j].x + nndx)*ucdx;
         double jy = (cs::unit_cell.atom[cs::unit_cell.bilinear.interaction_ptr[itr].j].y + nndy)*ucdy;
         double jz = (cs::unit_cell.atom[cs::unit_cell.bilinear.interaction_ptr[itr].j].z + nndz)*ucdz;
