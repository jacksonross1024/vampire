...
namespace unitcell{
   ...
   uint64_t unitcell::exchange_template_t::read_interactions_stack(
      //unit_cell_t& unit_cell,
      const uint64_t num_atoms, // num atoms in unit cell
      std::stringstream& ucf_file,
      std::istringstream& ucf_ss,
      ...
      // resize interaction counter array to store number of interactions per atom
      ni.resize(num_atoms, 0);
      // interaction_stack.resize(num_interactions);
      // loop over all interactions and read into class
      for (uint64_t i=0; i<num_interactions; i++){
         ...
         if(iatom>=0 && iatom < num_atoms && id < num_interactions) tmp.i=iatom;
         else if(iatom>=0 && iatom >= num_atoms){
            terminaltextcolor(RED);
            std::cerr << std::endl << "Error! iatom number "<< iatom <<" for interaction id " << id << " on line " << line_counter
                 << " of unit cell input file " << filename.c_str() << " is outside of valid range 0-"
         ...
         if(iatom>=0 && jatom < num_atoms) tmp.j=jatom;
         ...
         switch(num_exchange_values){
            case 1:
               int_iss >> tmp.Jij[0][0];
               // save interactions into diagonal components of the exchange tensor
               tmp.Jij[1][1] = tmp.Jij[0][0];
               tmp.Jij[2][2] = tmp.Jij[0][0];
               break;
            case 3:
               int_iss >> tmp.Jij[0][0] >> tmp.Jij[1][1] >> tmp.Jij[2][2];
               break;
            case 9:
               int_iss >> tmp.Jij[0][0] >> tmp.Jij[0][1] >> tmp.Jij[0][2];
               int_iss >> tmp.Jij[1][0] >> tmp.Jij[1][1] >> tmp.Jij[1][2];
               int_iss >> tmp.Jij[2][0] >> tmp.Jij[2][1] >> tmp.Jij[2][2];
         ...
         interaction_stack.push_back(tmp);
