...
void initialize () {
  ...
  //  namespace fs = std::filesystem; {
    ...
    std::cout << "E_f(J): " << E_f << ", TLE(J): " << (TLKE*1e-20*atomic_mass/2) + (std::accumulate(atom_potential.begin(), atom_potential.end(), 0)*1e10*constants::K) << ", TE(J): " << E_f*conduction_electrons << ", TEKE(AJ):" << TEKE*1e10*constants::m_e/2 << ", TEPE " << TEPE*1e10*constants::K << ", TEE " <<  ((TEKE*constants::m_e)/2 + (TEPE*constants::K))*1e10 << ", EE(J): " << constants::m_e*TEKE*0.5*1e10 + constants::K*1e10*std::accumulate(electron_potential.begin(), electron_potential.end(), 0) <<  std::endl;
...
void initialize_lattice() {
    ...
    for (int a = 0; a < lattice_atoms; ++a) {  
        atomic_nearest_atom_list[a].resize(0.1*conduction_electrons,-1);
...
void initialize_atomic_interactions() {
    ...
    double x_distance,y_distance,z_distance, length, force;
    ...
    for (int e = 0; e < lattice_atoms; e++) {
        ...
        for (int i = 0; i < lattice_atoms; i++) {
            ...
            atomic_nearest_atom_list[e][neighbor_count] = array_index_i;
            neighbor_count++;
    ...
    for (int e = 0; e < conduction_electrons; e++) {
        ...
        for (int a = 0; a < lattice_atoms; a++) {
          ...
          //  std::cout << force << std::endl;
           ...
           //  negative_PE += PE;
            electron_force[array_index]     += force * cos(theta)*sin(phi) * mu_r;
            electron_force[array_index + 1] += force * sin(theta)*sin(phi) * mu_r;
            electron_force[array_index + 2] += force * cos(phi) * mu_r;

            atom_force[array_index_a]     += -1*force * cos(theta)*sin(phi) * combined_mass;
            atom_force[array_index_a + 1] += -1*force * sin(theta)*sin(phi) * combined_mass;
            atom_force[array_index_a + 2] += -1*force * cos(phi) * combined_mass;
   ...
   // double atom_energy = atomic_phonon_energy[2*a];
    ...
    if (excitation_energy > 0) {
      ...
      // if(scattering_velocity < 0) std::cout << P << ", " << scattering_velocity << ", " << P - scattering_velocity <<  std::endl;
        ...
        double polar_scattering_angle = atan(Py/Px);
