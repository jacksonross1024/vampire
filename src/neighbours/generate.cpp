...
void list_t::generate( std::vector<cs::catom_t>& atom_array,    // array of atoms (as reference for speed)
   ...
   int64_t num_neighbours = d[0] * (d[1]) * (d[2]) * num_atoms_in_unit_cell;
      ...
      if(vmpi::master){
         ...
         sizeof(uint64_t)*total_neighbours/1.0e6 << " MB from <" << d[0]  << ", " << (d[1]) << ", " << (d[2]) << "> supercells with " << num_atoms_in_unit_cell << " each " << std::endl;
      }
   #else
   ...
   num_neighbours = (num_cells)*(exchange.num_interactions);
      ...
      if(vmpi::master){
         ...
         sizeof(neighbour_t)*total_neighbours/1.0e6 << " MB from " << num_cells << " cells with " << exchange.num_interactions \
